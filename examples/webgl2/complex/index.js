...
async function main() {
    ...
    for (const modelInfo of modelInfoSet) {
        ...
        console.log(`${modelInfo.name} loaded:`, gltf);
        
        // Process all meshes from the scene
        ...
        async function processNode(nodeIndex, parentMatrix) {
            ...
            if (node.mesh !== undefined) {
                const { primitives, bbox } = await processMesh(gl, gltf, buffers, baseUrl, node.mesh);
                
                // Create final matrix with model transform applied
                ...
                // Transform bbox to world space and merge
                const corners = [
    ...
    console.log('Scene bounding box:', sceneBbox);
    console.log('Center:', center, 'Size:', size, 'Camera distance:', cameraDistance);
    console.log('Total meshes processed:', allMeshData.length);
    ...
    function render() {
        ...
        for (const mesh of allMeshData) {
            // Use original matrix (camera orbits instead of model rotating)
